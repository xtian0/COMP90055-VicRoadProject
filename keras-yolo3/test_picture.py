...
# txt file for result record
#txt_path = 'test_out/result.txt'
#file = open(txt_path, 'w')


class YOLO(object):
    _defaults = {
        "model_path": 'logs/general_augmented/trained_weights_final.h5',
    ...
    def generate(self):
        ...
        try:
            ...
            assert self.yolo_moel.layers[-1].output_shape[-1] == \
    ...
    def detect_image(self, image):
        ...
        print(image_data.shape)  # print pic size
        image_data /= 255.
        ...
        print('Found {} boxes for {}'.format(len(out_boxes), 'img'))  # box num
...
if __name__ == '__main__':
    ...
    for filename in os.listdir(path):
        txt_path = 'test_out/out/'+filename.split('.')[0]+'.txt'
        file = open(txt_path, 'w')
        image_path = 'test_imgs/' + filename
        portion = os.path.split(image_path)
        #file.write(portion[1] + ' detect_result：\n')
        image = Image.open(image_path)
        r_image = yolo.detect_image(image)
        #file.write('\n')
        image_save_path = 'test_out/result_' + portion[1]
